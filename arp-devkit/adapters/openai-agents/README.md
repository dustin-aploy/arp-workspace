...
For OpenAI agent-style orchestration, ARP should sit around tool invocation, handoff, and outbound action boundaries. ARP is not meant to replace the framework planner; it is meant to govern what planned actions are allowed to proceed.
...
When the framework proposes a tool/action, when ARP blocks it, and when ARP escalates it. That keeps the governance trail attached to the agent lifecycle.
