...
A thin ARP adapter should expose only the minimum structure needed to:
...
2. **Policy gateway** — a callable surface that asks the ARP layer for pre-action checks, escalation routing, audit logging, and budget enforcement.
3. **Framework callback wrapper** — a tiny shim that translates framework lifecycle events into ARP-shaped calls.

## What adapters should not own

Adapters should not become the place where:

- ARP schemas are redefined;
