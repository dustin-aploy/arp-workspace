"""Minimal offline build backend for arp-test-suite."""
...
SRC = ROOT / "src"
PACKAGE_ROOT = SRC / "arp_test_suite"
DIST_NAME = "arp_test_suite"
PROJECT_NAME = "arp-test-suite"
VERSION = (ROOT / "VERSION").read_text(encoding="utf-8").strip()
PROTOCOL_DEPENDENCY_URI = (ROOT.parents[1] / "arp-spec").resolve().as_uri()
RUNTIME_DEPENDENCY_URI = (ROOT.parent / "runtime").resolve().as_uri()
DIST_INFO = f"{DIST_NAME}-{VERSION}.dist-info"
WHEEL_NAME = f"{DIST_NAME}-{VERSION}-py3-none-any.whl"
ENTRY_POINTS = "[console_scripts]\narp-test-suite = arp_test_suite.runner:main\n"
...
def _metadata_text() -> str:
    return "\n".join(
        [
            ...
            f"Name: {PROJECT_NAME}",
            f"Version: {VERSION}",
            "Summary: Conformance and certification foundation for the Agent Responsibility Protocol (ARP).",
            "Requires-Python: >=3.10",
            f"Requires-Dist: arp-protocol @ {PROTOCOL_DEPENDENCY_URI}",
            f"Requires-Dist: arp-runtime @ {RUNTIME_DEPENDENCY_URI}",
            "License: Apache-2.0",
            "",
        ]
    )
...
def _wheel_text() -> str:
    return "\n".join(
        [
            "Wheel-Version: 1.0",
            "Generator: arp-test-suite-build-backend",
...
def _build_metadata_dir(metadata_directory: str) -> str:
    ...
    (metadata_dir / "top_level.txt").write_text("arp_test_suite\n", encoding="utf-8")
...
def _write_wheel(wheel_directory: str, editable: bool) -> str:
    ...
    if editable:
        entries.append((f"{DIST_NAME}.pth", f"{SRC.resolve()}\n".encode("utf-8")))
    else:
    ...
    entries.extend(
        [
            ...
            (f"{DIST_INFO}/top_level.txt", b"arp_test_suite\n"),
        ]
    )
