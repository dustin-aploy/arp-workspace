...
from arp_test_suite.loader import declaration_digest, load_agent, load_json_fixture, load_protocol_version
from arp_test_suite.report import build_report, write_report
from arp_test_suite.result_types import CheckResult
...
class ComplianceRunner:
    ...
    def run(self, *, agent_path: str | Path, project: str, output_path: str | Path = "compliance-report.json") -> dict:
        ...
        summary = build_report(
            project=project,
            arp_spec_version=load_protocol_version(),
    ...
    def check_audit_fields(self, config: dict) -> CheckResult:
        audit_path = Path(tempfile.gettempdir()) / "arp-test-suite-audit-fields.jsonl"
    ...
    def check_audit_event_types(self, config: dict) -> CheckResult:
        audit_path = Path(tempfile.gettempdir()) / "arp-test-suite-audit-types.jsonl"
        runtime = self._runtime_with_config(config, audit_path=audit_path)
        runtime.evaluate("Draft a response", "draft_response", 0.95)
        payload = json.loads(audit_path.read_text(encoding="utf-8").splitlines()[-1])
    ...
    def check_audit_on_block(self, config: dict) -> CheckResult:
        audit_path = Path(tempfile.gettempdir()) / "arp-test-suite-audit-block.jsonl"
        runtime = self._runtime_with_config(config, audit_path=audit_path)
    ...
    def check_audit_on_escalation(self, config: dict) -> CheckResult:
        audit_path = Path(tempfile.gettempdir()) / "arp-test-suite-audit-escalation.jsonl"
        runtime = self._runtime_with_config(config, audit_path=audit_path)
    ...
    def check_daily_report_format(self, config: dict) -> CheckResult:
        ...
        passed = report.get("kind") == "ARPReport" and report.get("report_type") == "operational"
    ...
    def _runtime_with_config(self, config: dict, audit_path: str | Path | None = None) -> ARPRuntime:
        temp_path = Path(tempfile.gettempdir()) / f"arp-test-suite-{config['identity']['id'].replace('.', '-')}.yaml"
        temp_path.write_text(_dump_yaml(config), encoding="utf-8")
        return ARPRuntime(temp_path, audit_path=audit_path)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="arp-test-suite", description="ARP compliance runner")
