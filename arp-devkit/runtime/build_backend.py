"""Minimal offline build backend for arp-runtime.
...
SRC = ROOT / "src"
PACKAGE_ROOT = SRC / "arp_runtime"
DIST_NAME = "arp_runtime"
PROJECT_NAME = "arp-runtime"
...
ENTRY_POINTS = "[console_scripts]\narp-runtime = arp_runtime.cli:main\n"
...
def _metadata_text() -> str:
    return "\n".join(
        [
            ...
            f"Name: {PROJECT_NAME}",
            f"Version: {VERSION}",
            "Summary: Lightweight reference runtime for the Agent Responsibility Protocol (ARP).",
...
def _wheel_text() -> str:
    return "\n".join(
        [
            "Wheel-Version: 1.0",
            "Generator: arp-runtime-build-backend",
...
def _build_metadata_dir(metadata_directory: str) -> str:
    ...
    (metadata_dir / "top_level.txt").write_text("arp_runtime\n", encoding="utf-8")
...
def _write_wheel(wheel_directory: str, editable: bool) -> str:
    ...
    entries.extend(
        [
            ...
            (f"{DIST_INFO}/top_level.txt", b"arp_runtime\n"),
        ]
    )
