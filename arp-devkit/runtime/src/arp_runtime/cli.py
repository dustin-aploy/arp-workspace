...
from arp_runtime.validator.validator import SchemaValidationError

DEFAULT_AUDIT_LOG = "arp-runtime.audit.jsonl"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="arp-runtime", description="ARP reference runtime CLI")
    parser.add_argument("--agent", required=True, help="Path to an ARP YAML declaration")
    ...
    parser.add_argument("--audit-log", default=DEFAULT_AUDIT_LOG, help="JSONL audit log path (defaults to ./arp-runtime.audit.jsonl)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        runtime = ARPRuntime(args.agent, audit_path=args.audit_log)
