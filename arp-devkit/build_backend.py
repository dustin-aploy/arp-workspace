"""Minimal offline build backend for the grouped arp-devkit package."""
...
DIST_NAME = "arp_devkit"
PROJECT_NAME = "arp-devkit"
...
def _metadata_text() -> str:
    return "\n".join(
        [
            ...
            f"Name: {PROJECT_NAME}",
            f"Version: {VERSION}",
            "Summary: Grouped developer toolkit workspace for the Agent Responsibility Protocol (ARP).",
...
def _wheel_text() -> str:
    return "\n".join(
        [
            "Wheel-Version: 1.0",
            "Generator: arp-devkit-build-backend",
